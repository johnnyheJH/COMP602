...
if(isset($_POST['forgotPswSubmit'])){
    ...
    if(!empty($_POST['email'])){
        ...
        if($prevUser > 0){
            ...
            if($update){
                $resetPassLink = 'http://unishare.com/resetPassword.php?fp_code='.$uniqidStr; //link need to be changed
                ...
                <br/>To reset your password, click on the following link: <a href="'.$resetPassLink.'">'.$resetPassLink.'</a>
                <br/><br/>Regards,
                <br/>UniSahre';
                ...
                //additional headers
                $headers .= 'From: UniSHare<unisharee@gmail.com>' . "\r\n";
                //send email
    ...
    if(!empty($_POST['password']) && !empty($_POST['confirm_password']) && !empty($_POST['fp_code'])){
        ...
        if($_POST['password'] !== $_POST['confirm-psw']){
            $sessData['status']['type'] = 'error';
